...
// --- Global State ---
let w, h, time = 0, lastTime = 0;
let leaves = [], grass = [], mistParticles = [], waterRipples = [];
let mountains = { far: [], near: [] };
    ...
    measureAndSize() {
        try {
            ...
            if (lines.length > CONFIG.leafMaxLines) {
                ...
                lines[CONFIG.leafMaxLines - 1] = lines[CONFIG.leafMaxLines - 1].slice(0, -3) + '…';
            }
            ...
            this.leafWidth = Math.max(CONFIG.leafMinWidth, maxLineWidth + CONFIG.leafPadding);
...
this.leafHeight = Math.max(45, textBlockHeight + verticalPadding);
    ...
    resetPosition() {
        try {
            ...
            this.x = w * 0.5; this.y = -80; this.progress = 0;
        }
    }
    ...
    update(deltaTime) {
        try {
            ...
            const speed = CONFIG.baseSpeed + (this.progress * CONFIG.accelerationFactor);
            this.progress += speed * dt;
            this.y = this.progress * h;
            
            // Gentle drift + wobble
            this.x += this.vx + Math.sin(time * 0.015 + this.y * 0.006 + this.wobblePhase) * 0.25;
            this.angle = this.baseAngle + Math.sin(this.wobblePhase + time * this.wobbleSpeed) * 0.15;
...
const safeHeight = this.leafHeight * 0.75;
const textBlockHeight = this.lineCount * CONFIG.leafLineHeight;
...
this.lines.forEach((line, idx) => {
    ...
    if (metrics.width > safeWidth) {
        while (ctx.measureText(displayLine + '…').width > safeWidth && displayLine.length > 4) {
...
function releaseLeaf() {
    try {
        ...
        if (!text) {
            ...
            setTimeout(() => { input.style.animation = null; }, 200);
            return;
        }
...
function drawWater() {
    try {
        ...
        for (let i = 0; i < 14; i++) {
            ...
            ctx.quadraticCurveTo(xPos + curve, offset + 35, xPos + curve * 0.5, offset + 70);
            ctx.stroke();
        }
        ...
        waterRipples.forEach(ripple => {
            ...
            ctx.lineWidth = 0.7;
            ctx.beginPath();
            ctx.ellipse(ripple.x, ripple.y, ripple.radius * (0.8 + pulse * 0.3), ripple.radius * 0.25, 0, Math.PI * 2);
            ctx.stroke();
        });
        
        // Waterfall foam zone
        const foamGrad = ctx.createLinearGradient(0, h * CONFIG.waterfallStart, 0, h);
        ...
        ctx.fillRect(riverX, h * CONFIG.waterfallStart, riverW, h * (1 - CONFIG.waterfallStart));
    } catch (err) {
...
function drawGrass(deltaTime) {
    try {
        ...
        grass.forEach(g => {
            const swayAmount = g.sways ? Math.sin(time * g.swaySpeed + g.phase) * 6 : 0;
...
function drawBreathPulse() {
    try {
        const breathProgress = (Date.now() % CONFIG.breathCycleMs) / CONFIG.breathCycleMs;
