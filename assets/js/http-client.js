...
(function () {
  ...
  function createApiClient() {
    if (typeof ky === 'undefined') {
      console.warn('[HttpClient] Ky library not loaded, using native fetch fallback.');
      return null;
    }
    ...
    try {
      const api = ky.create({
        ...
        hooks: {
          beforeRetry: [
            ({ request, retryCount }) => {
              console.log(`[HttpClient] Retry ${retryCount} for ${request.url}`);
            }
          ],
          afterResponse: [
            (request, options, response) => {
              if (!response.ok) {
                console.error(`[HttpClient] HTTP Error: ${response.status} ${response.statusText} for ${request.url}`);
              }
              return response;
            }
          ]
  ...
  async function nativeFetchWithTimeout(url, options = {}, timeout = 8000) {
    ...
    try {
      ...
      if (error.name === 'AbortError') {
        throw new Error(`[HttpClient] Request to ${url} timed out after ${timeout}ms`);
      }
      throw error;
    }
  }
  ...
  async function fetchWithRetry(url, options = {}) {
    ...
    if (apiInstance) {
      try {
        ...
        console.error('[HttpClient] Ky request failed, falling back to native fetch:', error);
      }
    }
  ...
  async function getJson(url, options = {}) {
    ...
    if (!response.ok) {
      throw new Error(`[HttpClient] Failed to fetch ${url}: ${response.status} ${response.statusText}`);
    }
  ...
  async function postJson(url, data, options = {}) {
    ...
    if (!response.ok) {
      throw new Error(`[HttpClient] Failed to post to ${url}: ${response.status} ${response.statusText}`);
    }
    return response.json();
  }
  ...
  window.HttpClient = {
    get api() { return apiInstance || createApiClient(); },
