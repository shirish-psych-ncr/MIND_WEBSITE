...
;(() => {
  ...
  class GalleryLightbox {
    ...
    _injectControls() {
      // Close button
      this.closeBtn = this._createButton('gallery-close-btn', 'Close image viewer', `
      ...
      // Append all to body
      const controls = [this.closeBtn, this.prevBtn, this.nextBtn, this.counter, this.captionOverlay];
      controls.forEach(el => {
    ...
    _bindEvents() {
      ...
      this.cards.forEach((card, index) => {
        ...
        card.setAttribute('aria-label',
          card.querySelector('figcaption')?.textContent || `Gallery image ${index + 1}`
        );
    ...
    open(index) {
      ...
      const caption = this.cards[index].querySelector('figcaption')?.textContent || '';
      announce(`Image viewer opened. ${caption}. Image ${index + 1} of ${this.cards.length}.`);
    }
    ...
    _updateDisplay() {
      ...
      const caption = this.cards[this.currentIndex].querySelector('figcaption')?.textContent || '';
