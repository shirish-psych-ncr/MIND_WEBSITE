...
document.addEventListener("DOMContentLoaded", () => {
	...
	function initMobileNav(el) {
		const {
			mobileNavTrigger: trigger,
			mobileNavPanel: panel,
		...
		const prefersCompact =
			localStorage.getItem("compact-nav-preference") === "true";
					...
					if (!fieldIsValid) {
						...
						if (!field.parentNode.querySelector(".error-message")) {
							...
							errorMsg.textContent =
								state.isEmail && state.value
									? "Please enter a valid email"
									: "This field is required";
							errorMsg.style.cssText =
								"color: var(--error); font-size: 0.75rem; margin-top: var(--space-xs); display: block;";
			...
			(entries, obs) => {
				entries.forEach((entry) => {
					if (entry.isIntersecting) {
						...
						const target =
							parseInt(counter.getAttribute("data-target"), 10) || 100;
...
function initOpenGraphMeta() {
	...
	const defaultDescription =
		"Compassionate, evidence-based neuropsychiatric care in India. Specialized treatment for anxiety, depression, ADHD, and more.";
	...
	const ogTags = [
		{
			property: "og:title",
			content: `${pageTitle} | Mind Grace Neuropsychiatric Clinic`,
		},
