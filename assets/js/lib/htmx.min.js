const _htmx = (() => {
	...
	const T = de.map((e) => `[hx-${e}], [data-hx-${e}]`).join(", ");
	function d(e) {
		if (e === undefined) {
			return undefined;
		}
	...
	function qe(e) {
		ie(x(e, "[hx-preserve], [data-hx-preserve]"), (e) => {
			const t = a(e, "id");
	...
	function Ae(l, e, c) {
		ie(e.querySelectorAll("[id]"), (t) => {
			...
			if (n && n.length > 0) {
				...
				const e = p(l);
				const i = e?.querySelector(`${o}[id='${r}']`);
				if (i && i !== e) {
					const s = t.cloneNode();
					Oe(t, i);
					c.tasks.push(() => {
	...
	function _e(e, n, r) {
		const t = x(e, "[hx-swap-oob], [data-hx-swap-oob]");
		ie(t, (e) => {
	...
	function ht(e, t) {
		if (e.type === "submit" && t.tagName === "FORM") {
			...
			if (n?.form && n.type === "submit") {
				return true;
			}
			const r = t.closest("a");
			const o = /^#.+/;
			if (r?.href && !o.test(r.getAttribute("href"))) {
				return true;
			}
		}
		return false;
				...
				if (t.handledFor.indexOf(l) < 0) {
					...
					if (u.throttle > 0) {
						if (!a.throttle) {
							ae(l, "htmx:trigger");
							c(l, e);
							a.throttle = b().setTimeout(() => {
								a.throttle = null;
							}, u.throttle);
						}
					} else if (u.delay > 0) {
						a.delayed = b().setTimeout(() => {
							ae(l, "htmx:trigger");
							c(l, e);
						}, u.delay);
					} else {
	...
	function Kt(e) {
		...
		ie(x(n, "[data-disabled-by-htmx]"), (e) => {
	...
	function Mn(e, t) {
		const n = new RegExp(e.code);
		return n.test(t.toString(10));
	}
	function Xn(e) {
		for (let t = 0; t < Q.config.responseHandling.length; t++) {
			const n = Q.config.responseHandling[t];
			if (Mn(n, e.status)) {
				return n;
			}
		}
	...
	function $n() {
		return {
			...
			getSelectors: () => null,
			onEvent: (_e, _t) => true,
			transformResponse: (e, _t, _n) => e,
			isInlineSwap: (_e) => false,
			handleSwap: (_e, _t, _n, _r) => false,
			encodeParameters: (_e, _t, _n) => null,
		};
	}
	function _n(e, t) {
		if (t.init) {
