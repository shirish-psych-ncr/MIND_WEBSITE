(() => {
    ...
    const effectReference = effect(() => {
      ...
      if (!firstTime) {
        if (typeof value === "object" || value !== oldValue) {
          const previousValue = typeof oldValue === "object" ? JSON.parse(oldValueJSON) : oldValue;
          queueMicrotask(() => {
  ...
  function initInterceptors(data2) {
    const isObject3 = (val) => typeof val === "object" && !Array.isArray(val) && val !== null;
...
${expression ? 'Expression: "' + expression + '"\n\n' : ""}`, el);
  ...
  function generateEvaluatorFromString(dataStack, expression, el) {
    ...
    return (receiver = () => {
      ...
      if (typeof func === "function") {
        const promise = func.call(context, func, completeScope).catch((error2) => handleError(error2, el, expression));
        if (func.finished) {
  ...
  function normalRawEvaluator(el, expression, extras = {}) {
    ...
    injectMagics(overriddenMagics, el);
    const dataStack = [overriddenMagics, ...closestDataStack(el)];
    const scope2 = mergeProxies([extras.scope ?? {}, ...dataStack]);
    ...
    if (expression.includes("await")) {
      ...
      const result = func.call(extras.context, scope2);
      return result;
    } else {
  ...
  function setClassesFromString(el, classString) {
    const missingClasses = (classString2) => splitClasses(classString2).filter((i) => !el.classList.contains(i)).filter(Boolean);
  ...
  function setClassesFromObject(el, classObject) {
    const forAdd = Object.entries(classObject).flatMap(([classString, bool]) => bool ? splitClasses(classString) : false).filter(Boolean);
    const forRemove = Object.entries(classObject).flatMap(([classString, bool]) => !bool ? splitClasses(classString) : false).filter(Boolean);
    const added = [];
    const removed = [];
    forRemove.forEach((i) => {
  ...
  const toReactive = (value) => isObject(value) ? reactive2(value) : value;
  ...
  directive("modelable", (el, { expression }, { effect: effect3, evaluateLater: evaluateLater2, cleanup: cleanup2 }) => {
    ...
    const innerSet = (val) => evaluateInnerSet(() => {
    ...
    queueMicrotask(() => {
      ...
      const outerSet = el._x_model.setWithModifiers;
  ...
  directive("model", (el, { modifiers, expression }, { effect: effect3, cleanup: cleanup2 }) => {
    ...
    const hasChangeModifier = modifiers.includes("change") || modifiers.includes("lazy");
    const hasBlurModifier = modifiers.includes("blur");
    const hasEnterModifier = modifiers.includes("enter");
    const hasExplicitEventModifiers = hasChangeModifier || hasBlurModifier || hasEnterModifier;
    let removeListener;
    if (isCloning) {
      removeListener = () => {
      };
      ...
      const listeners = [];
      const syncValue = (e) => setValue(getInputValue(el, modifiers, e, getValue()));
      ...
      if (hasBlurModifier) {
        ...
        if (el.form) {
          const form = el.form;
          const syncCallback = () => syncValue({ target: el });
      ...
      const event = el.tagName.toLowerCase() === "select" || ["checkbox", "radio"].includes(el.type) ? "change" : "input";
  ...
  const handler2 = (el, { value, modifiers, expression, original }, { effect: effect3, cleanup: cleanup2 }) => {
    if (!value) {
  ...
  function loop(templateEl, iteratorNames, evaluateItems, evaluateKey) {
    evaluateItems((items) => {
      ...
      const hasStringKeys = isObject2(items);
      const scopeEntries = Object.entries(items).map(([index, item]) => {
        if (!hasStringKeys)
          index = parseInt(index);
        const scope2 = getIterationScopeVariables(iteratorNames, item, index, items);
        let key;
  ...
  function getIterationScopeVariables(iteratorNames, item, index, items) {
    ...
    if (/^\[.*\]$/.test(iteratorNames.item) && Array.isArray(item)) {
      const names = iteratorNames.item.replace("[", "").replace("]", "").split(",").map((i) => i.trim());
  ...
  const src_default = alpine_default;
