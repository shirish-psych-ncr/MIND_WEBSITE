...
${expression ? `Expression: "${expression}"\n\n` : ""}`,
	...
	function registerTransitionsFromHelper(el, modifiers, stage) {
		...
		if (modifiers.includes("in") && !doesntSpecify) {
			modifiers = modifiers.filter(
				(_i, index) => index < modifiers.indexOf("out"),
			);
		}
	...
	function modifierValue(modifiers, key, fallback) {
		...
		if (key === "scale") {
			if (Number.isNaN(rawValue)) return fallback;
		}
	...
	function entangle(
		...
		const reference = effect(() => {
			...
			_innerHash = JSON.stringify(innerGet());
		});
		return () => {
			release(reference);
		};
	...
	const _isBooleanAttr2 = /* @__PURE__ */ makeMap(
		specialBooleanAttrs +
			`,async,autofocus,autoplay,controls,default,defer,disabled,hidden,loop,open,required,reversed,scoped,seamless,checked,muted,multiple,selected`,
	);
	const EMPTY_OBJ = true ? Object.freeze({}) : {};
	const _EMPTY_ARR = true ? Object.freeze([]) : [];
	...
	const _camelize = cacheStringFunction((str) => {
		return str.replace(camelizeRE, (_, c) => (c ? c.toUpperCase() : ""));
	});
	const hyphenateRE = /\B([A-Z])/g;
	const _hyphenate = cacheStringFunction((str) =>
		str.replace(hyphenateRE, "-$1").toLowerCase(),
	);
	const capitalize = cacheStringFunction(
		(str) => str.charAt(0).toUpperCase() + str.slice(1),
	);
	const _toHandlerKey = cacheStringFunction((str) =>
	...
	magic(
		"watch",
		(_el, { evaluateLater: evaluateLater2, cleanup: cleanup2 }) =>
			(key, callback) => {
	...
	function closestIdRoot(el, name) {
		return findClosest(el, (element) => {
			if (element._x_ids?.[name]) return true;
		});
	}
	...
	function isNumeric(subject) {
		return !Array.isArray(subject) && !Number.isNaN(subject);
	}
	...
	directive(
		...
		(el, { modifiers, expression }, { effect: effect3, cleanup: cleanup2 }) => {
			...
			const hasExplicitEventModifiers =
			...
			if (el.form) {
				const removeResetListener = on(el.form, "reset", [], (_e) => {
					nextTick(() =>
						el._x_model?.set(
	...
	directive(
		"init",
		skipDuringClone((_el, { expression }, { evaluate: evaluate2 }) => {
			if (typeof expression === "string") {
	...
	const handler2 = (
		...
		if (el._x_inlineBindings?.[value]?.extract) {
			return;
		}
	...
	function loop(templateEl, iteratorNames, evaluateItems, evaluateKey) {
		evaluateItems((items) => {
			...
			const scopeEntries = Object.entries(items).map(([index, item]) => {
				if (!hasStringKeys) index = parseInt(index, 10);
	...
	function isNumeric3(subject) {
		return typeof subject !== "object" && !Number.isNaN(subject);
	}
