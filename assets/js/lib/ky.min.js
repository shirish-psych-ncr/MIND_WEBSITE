...
const l = (s) => s !== null && typeof s === "object",
	f = (...s) => {
		for (const t of s)
	...
	y = (...s) => {
		let t = {},
			r = {};
		for (const e of s)
			if (Array.isArray(e)) Array.isArray(t) || (t = []), (t = [...t, ...e]);
			else if (l(e)) {
				for (let [o, n] of Object.entries(e))
					l(n) && o in t && (n = y(t[o], n)), (t = { ...t, [o]: n });
				l(e.headers) && ((r = w(r, e.headers)), (t.headers = r));
			}
		return t;
	},
	C = (() => {
		...
		const r = typeof globalThis.ReadableStream === "function",
			e = typeof globalThis.Request === "function";
		return (
			r &&
				e &&
	...
	A = typeof globalThis.AbortController === "function",
	E = typeof globalThis.ReadableStream === "function",
	S = typeof globalThis.FormData === "function",
	...
	m = 2147483647,
	T = Symbol("stop"),
	P = (s) => (g.includes(s) ? s.toUpperCase() : s),
	O = ["get", "put", "head", "delete", "options", "trace"],
	k = [408, 413, 429, 500, 502, 503, 504],
	R = [413, 429, 503],
	q = {
		limit: 2,
		methods: O,
	...
	j = (s = {}) => {
		...
		return { ...q, ...s, afterStatusCodes: R };
	};
async function I(s, t, r) {
...
const U = !!globalThis.DOMException;
function x(s) {
	if (U)
		return new DOMException(
			s?.reason ?? "The operation was aborted.",
			"AbortError",
		);
	const t = new Error(s?.reason ?? "The operation was aborted.");
	return (t.name = "AbortError"), t;
}
async function L(s, { signal: t }) {
...
class p {
	static create(t, r) {
		const e = new p(t, r),
			o = async () => {
				if (e._options.timeout > m)
					throw new RangeError(
						`The \`timeout\` option cannot be greater than ${m}`,
					);
				await Promise.resolve();
				let i = await e._fetch();
				for (const u of e._options.hooks.afterResponse) {
					const h = await u(
						e.request,
						e._options,
						e._decorateResponse(i.clone()),
					);
					h instanceof globalThis.Response && (i = h);
				}
				if ((e._decorateResponse(i), !i.ok && e._options.throwHttpErrors)) {
					let u = new d(i, e.request, e._options);
					for (const h of e._options.hooks.beforeError) u = await h(u);
					throw u;
				}
			...
			a = e._options.retry.methods.includes(e.request.method.toLowerCase())
				? e._retry(o)
				: o();
		for (const [i, u] of Object.entries(v))
			a[i] = async () => {
				e.request.headers.set("accept", e.request.headers.get("accept") || u);
				...
				if (i === "json") {
					if (
						...
						return "";
					if (r.parseJson) return r.parseJson(await c.text());
				}
				return c[i]();
			};
		return a;
	...
	constructor(t, r = {}) {
		if (
			...
			(this._options = {
				...
				...r,
				headers: w(this._input.headers, r.headers),
				hooks: y(
					{
						beforeRequest: [],
						beforeRetry: [],
				...
				method: P(r.method ?? this._input.method),
				prefixUrl: String(r.prefixUrl || ""),
				retry: j(r.retry),
				throwHttpErrors: r.throwHttpErrors !== !1,
				timeout: typeof r.timeout > "u" ? 1e4 : r.timeout,
		...
		if (
			...
			const o =
				...
				n = this.request.url.replace(/(?:\?.*?)?(?=#|$)/, o);
			((S && this._options.body instanceof globalThis.FormData) ||
				...
				(this.request = new globalThis.Request(
					new globalThis.Request(n, { ...this.request }),
					this._options,
				));
		}
	...
	_calculateRetryDelay(t) {
		if (
			(this._retryCount++,
			this._retryCount < this._options.retry.limit && !(t instanceof b))
		) {
			if (t instanceof d) {
				...
				if (
					...
					let o = Number(e);
					return (
						Number.isNaN(o) ? (o = Date.parse(e) - Date.now()) : (o *= 1e3),
						typeof this._options.retry.maxRetryAfter < "u" &&
						o > this._options.retry.maxRetryAfter
							? 0
							: o
					);
				}
	...
	async _retry(t) {
		try {
			...
			const e = Math.min(this._calculateRetryDelay(r), m);
			if (e !== 0 && this._retryCount > 0) {
				await L(e, { signal: this._options.signal });
	...
	async _fetch() {
		...
		return this._options.timeout === !1
			...
			: I(this.request.clone(), this.abortController, this._options);
	}
	_stream(t, r) {
		...
		return t.status === 204
			? (r?.(
					{ percent: 1, totalBytes: e, transferredBytes: o },
					new Uint8Array(),
				),
			...
			: new globalThis.Response(
					new globalThis.ReadableStream({
						async start(n) {
							...
							r?.(
								{ percent: 0, transferredBytes: 0, totalBytes: e },
								new Uint8Array(),
							);
							async function i() {
								...
								if (r) {
									o += h.byteLength;
									const c = e === 0 ? 0 : o / e;
									r({ percent: c, transferredBytes: o, totalBytes: e }, h);
								}
								n.enqueue(h), await i();
							}
							await i();
...
const _ = (s) => {
		const t = (r, e) => p.create(r, f(s, e));
		for (const r of g) t[r] = (e, o) => p.create(e, f(s, o, { method: r }));
		return (
			(t.create = (r) => _(f(r))),
			(t.extend = (r) => _(f(s, r))),
			(t.stop = T),
...
export { b as TimeoutError, d as HTTPError, N as default };
