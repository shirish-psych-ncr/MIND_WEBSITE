		...
		(i = Object.assign
			...
			: function (t) {
					for (let e = 1; e < arguments.length; e++) {
						const i = arguments[e];
						for (const s in i) Object.hasOwn(i, s) && (t[s] = i[s]);
					}
					return t;
				}),
...
async function u(t, e = {}) {
	...
	if (!w) throw new c(`Empty response: ${g}`, { status: m, url: g });
	const { url: f } = l.fromUrl(g),
		v = { url: f, html: w };
	return (
		!n.cache.write ||
			(e.method && "GET" !== e.method) ||
			t !== f ||
			this.cache.set(v.url, v),
		v
	);
}
class d {
...
function y(t, e) {
	...
	return i?.hasAttribute(e)
		? (null == i ? void 0 : i.getAttribute(e)) || !0
		: void 0;
}
class k {
	constructor(t) {
...
class E {
	...
	on(t, e, s = {}) {
		...
		const o = i({}, s, { id: ++this.nextHookId, hook: t, handler: e });
	...
	parseCallArgs(_t, e, i, s) {
		return e instanceof b || ("object" !== typeof e && "function" !== typeof i)
			? [e, i, s]
			: [void 0, e, i];
	}
	...
	getHandlers(t, e) {
		...
		if (e && ((h = [{ id: 0, hook: t, handler: e }]), l)) {
			const i = r.length - 1,
				{ handler: s, once: n } = r[i],
				o = (t) => {
					const i = r[t - 1];
					return i ? (e, s) => i.handler(e, s, o(t - 1)) : e;
				};
			h = [{ id: 0, hook: t, once: n, handler: s, defaultHandler: o(i) }];
		}
...
const C = (t) => {
		...
		let i =
			...
			p(`a[name='${CSS.escape(t)}']`) ||
			p(`a[name='${CSS.escape(e)}']`);
...
async function P({ selector: t, elements: e }) {
	...
	let i = [];
	if (e) i = Array.from(e);
	else if (t && ((i = m(t, document.body)), !i.length))
...
async function V(t, e = {}) {
	...
	let h, c;
	"string" === typeof a && (t.animation.name = a),
		(t.meta = e.meta || {}),
		"object" === typeof e.cache
			? ((t.cache.read = null != (h = e.cache.read) ? h : t.cache.read),
				(t.cache.write = null != (c = e.cache.write) ? c : t.cache.write))
	...
	try {
		...
		if (
			(t.history.popstate ||
				("replace" === t.history.action || t.to.url === this.location.url
					? r(n)
					: (this.currentHistoryIndex++,
						o(n, { index: this.currentHistoryIndex }))),
			(this.location = l.fromUrl(n)),
		...
		if (!e || e?.aborted) return void t.advance(8);
		t.advance(9),
			console.error(e),
	...
	O = function (t) {
		...
		if (((e = t), Boolean(null == e ? void 0 : e.isSwupPlugin))) {
			if (((t.swup = this), !t._checkRequirements || t._checkRequirements()))
				return (
					t._beforeMount?.(), t.mount(), this.plugins.push(t), this.plugins
				);
...
function M(t) {
	return this.plugins.find((e) =>
		"string" === typeof t ? [`Swup${t}`, t].includes(e.name) : e === t,
	);
}
function W(t) {
...
class _ {
	...
	constructor(t = {}) {
		...
		(this.version = "4.9.2"),
			...
			(this.hooks = void 0),
			(this.classes = void 0),
			(this.location = l.fromUrl(window.location.href)),
			...
			(this.log = () => {}),
			(this.navigate = I),
			(this.performNavigation = V),
			(this.createVisit = S),
			(this.delegateEvent = a),
			(this.fetchPage = u),
			(this.awaitAnimations = P),
			(this.renderPage = N),
			(this.replaceContent = q),
			(this.animatePageIn = T),
			(this.animatePageOut = L),
			(this.scrollToContent = R),
			(this.getAnchorElement = C),
			...
			(this.hooks = new E(this)),
			(this.visit = this.createVisit({ to: "" })),
			(this.currentHistoryIndex =
				null != (e = null == (s = window.history.state) ? void 0 : s.index)
					? e
					: 1),
			this.enable();
	}
	...
	async destroy() {
		this.clickDelegate.destroy(),
			...
			this.cache.clear(),
			this.plugins.forEach((t) => this.unuse(t)),
