...
const e = /*#__PURE__*/ t(require("delegate-it"));
const n = (t, e) =>
		String(t)
			.toLowerCase()
			.replace(/[\s/_.]+/g, "-")
	...
	r = (t, n, i, s) => {
		const { base: o = document, ...r } = void 0 === s ? {} : s;
		const a = e.default(o, t, n, i, r);
		return { destroy: () => a.destroy() };
	},
	a = (t, e) => (void 0 === e && (e = document), e.querySelector(t)),
	l = (t, e) => (
	...
	u = (t) => (window.CSS?.escape ? CSS.escape(t) : t),
	h = (t) => 1e3 * Number(t.slice(0, -1).replace(",", ".")),
	d = (t, e) => {
		...
		e.forEach((t) => {
			...
			l(t).length !== l(t, n).length &&
				console.warn(
					"[swup] Mismatched number of containers found on new page.",
				),
				l(t).forEach((_e, s) => {
					l(t, n)[s].setAttribute("data-swup", String(i.length)),
						i.push(l(t, n)[s].outerHTML);
				});
		});
		const s = a("title", n)?.innerText || "",
			o = a("body", n)?.className;
		return (
			(n.innerHTML = ""),
			(n = null),
...
const m = (t, e) => {
		...
		e.forEach((e) => {
			null == a(e, t)
				...
				: l(e).forEach((_i, s) => {
						l(e, t)[s].setAttribute("data-swup", String(n)), n++;
					});
		});
	},
	f = (t) =>
...
const E = function (t) {
		const { event: e, skipTransition: n } = void 0 === t ? {} : t;
		if (n)
			return (
		...
		? n.map((t) =>
				((t, e, n) => {
					...
					return i && s
						...
						: (console.warn(
								`[swup] No CSS transition duration defined for element of selector ${e}`,
							),
							Promise.resolve());
				})(t, e),
			)
		: (console.warn(`[swup] No animated elements found by selector ${e}`),
			[Promise.resolve()]);
}
function L(t, e) {
...
void 0 === window.ontransitionend &&
	...
	((S = "WebkitTransition"), (b = "webkitTransitionEnd")),
	void 0 === window.onanimationend &&
		void 0 !== window.onwebkitanimationend &&
		((k = "WebkitAnimation"), (U = "webkitAnimationEnd"));
const C = function (t) {
	const e = d(t.responseText, this.options.containers);
	return e
	...
	return this.cache.exists(n)
		...
		: new Promise((i, s) => {
				p({ ...t, headers: e }, (t) => {
					...
					if (!e?.blocks.length) return void s(n);
					const o = { ...e, url: n };
...
function R(t) {
	const { url: e, event: o, customTransition: r } = t ?? {},
		a = o instanceof PopStateEvent,
		l = this.shouldSkipTransition({ url: e, event: o });
	this.triggerEvent("transitionStart", o),
		this.updateTransition(i(), e, r),
		null != r && document.documentElement.classList.add(`to-${n(r)}`);
	const c = this.leavePage({ event: o, skipTransition: l }),
		u = this.fetchPage(t);
	a || s(e + (this.scrollToElement || "")),
		(this.currentPageUrl = i()),
		Promise.all([u, ...c])
			.then((t) => {
				const [e] = t;
				this.renderPage(e, { event: o, skipTransition: l });
			})
			.catch((t) => {
				void 0 !== t &&
...
const D = function (t) {
	if (t?.isSwupPlugin) {
		if (((t.swup = this), !t._checkRequirements || t._checkRequirements()))
			return t._beforeMount?.(), t.mount(), this.plugins.push(t), this.plugins;
...
const N = function (t, e) {
	const { event: n, skipTransition: s } = void 0 === e ? {} : e;
	if (
		(document.documentElement.classList.remove("is-leaving"),
		!this.isSameResolvedUrl(i(), t.url))
	)
		return;
	const { url: r } = g.fromUrl(t.responseURL);
	this.isSameResolvedUrl(i(), r) ||
		(this.cache.cacheUrl({ ...t, url: r }), (this.currentPageUrl = i()), o(r)),
		s || document.documentElement.classList.add("is-rendering"),
		...
		this.replaceContent(t).then(() => {
			this.triggerEvent("contentReplaced", n),
				...
				this.enterPage({ event: n, skipTransition: s }),
...
(exports.Location = g),
	(exports.classify = n),
	(exports.cleanupAnimationClasses = v),
	(exports.createHistoryRecord = s),
	(exports.default = class {
		constructor(t) {
			void 0 === t && (t = {}),
				...
				(this.on = $),
				(this.off = _),
				(this.updateTransition = W),
				(this.shouldSkipTransition = O),
				(this.getAnimationPromises = y),
				(this.getPageData = C),
				(this.fetchPage = T),
				...
				(this.unuse = I),
				(this.findPlugin = M),
				(this.getCurrentUrl = i),
				(this.cleanupAnimationClasses = v),
				(this.defaults = {
		...
		enable() {
			"undefined" !== typeof Promise
				? ((this.delegatedListeners.click = r(
					...
					m(document.documentElement, this.options.containers),
		...
		shouldIgnoreVisit(t, e) {
			const { el: n, event: i } = void 0 === e ? {} : e;
			const { origin: s, url: o, hash: r } = g.fromUrl(t);
			return (
		...
		linkClickHandler(t) {
			const e = t.delegateTarget,
				{ href: n, url: s, hash: o } = g.fromElement(e);
			...
			if (
				(this.triggerEvent("clickLink", t), t.preventDefault(), !s || s === i())
			)
				return void this.handleLinkToSamePage(s, o, t);
			if (this.isSameResolvedUrl(s, i())) return;
			this.scrollToElement = o || null;
			const r = e.getAttribute("data-swup-transition") || void 0;
			this.performPageLoad({ url: s, customTransition: r });
		}
		handleLinkToSamePage(t, e, n) {
			if (e) {
				if ((this.triggerEvent("samePageWithHash", n), !P(e)))
	...
	(exports.delegateEvent = r),
	(exports.escapeCssIdentifier = u),
	(exports.fetch = p),
	(exports.getCurrentUrl = i),
	(exports.getDataFromHtml = d),
	(exports.markSwupElements = m),
	(exports.nextTick = c),
	(exports.query = a),
	(exports.queryAll = l),
	(exports.toMs = h),
	(exports.updateHistoryRecord = o);
