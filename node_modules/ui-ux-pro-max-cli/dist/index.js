...
// src/types/index.ts
var AI_TYPES = ["claude", "cursor", "windsurf", "antigravity", "copilot", "roocode", "kiro", "codex", "qoder", "gemini", "trae", "opencode", "continue", "codebuddy", "droid", "kilocode", "warp", "augment", "all"];
var AI_FOLDERS = {
  claude: [".claude"],
...
var EXCLUDED_FILES = ["settings.local.json"];
...
async function copyFolders(sourceDir, targetDir, aiType) {
  const copiedFolders = [];
  const foldersToCopy = aiType === "all" ? Object.values(AI_FOLDERS).flat() : AI_FOLDERS[aiType];
...
import { readFile, mkdir as mkdir2, writeFile, cp as cp2, access as access2, readdir as readdir2, lstat, rm as rm2 } from "node:fs/promises";
import { join as join2, dirname } from "node:path";
import { homedir } from "node:os";
import { fileURLToPath } from "node:url";
var __dirname2 = dirname(fileURLToPath(import.meta.url));
var ASSETS_CANDIDATES = [
  join2(__dirname2, "..", "assets"),
  join2(__dirname2, "..", "..", "assets")
];
var ASSETS_DIR = ASSETS_CANDIDATES.find((path) => existsSync(path)) ?? ASSETS_CANDIDATES[0];
var AI_TO_PLATFORM = {
  claude: "claude",
  cursor: "cursor",
  windsurf: "windsurf",
...
async function loadPlatformConfig(aiType) {
  ...
  const configPath = join2(ASSETS_DIR, "templates", "platforms", `${platformName}.json`);
  const content = await readFile(configPath, "utf-8");
  return JSON.parse(content);
}
async function loadTemplate(templateName) {
  const templatePath = join2(ASSETS_DIR, "templates", templateName);
...
async function renderSkillFile(config, isGlobal = false) {
...
` + quickReferenceContent : "";
  content = content.replace(/\{\{TITLE\}\}/g, config.title).replace(/\{\{DESCRIPTION\}\}/g, config.description).replace(/\{\{SCRIPT_PATH\}\}/g, config.scriptPath).replace(/\{\{SKILL_OR_WORKFLOW\}\}/g, config.skillOrWorkflow).replace(/\{\{QUICK_REFERENCE\}\}/g, quickRefWithNewline);
  if (isGlobal) {
...
async function copyDataAndScripts(targetSkillDir) {
  const dataSource = join2(ASSETS_DIR, "data");
  const scriptsSource = join2(ASSETS_DIR, "scripts");
  const dataTarget = join2(targetSkillDir, "data");
  const scriptsTarget = join2(targetSkillDir, "scripts");
  if (await exists2(dataSource)) {
    await ensureCleanDir(dataTarget);
    await mkdir2(dataTarget, { recursive: true });
    await cp2(dataSource, dataTarget, { recursive: true });
  }
  if (await exists2(scriptsSource)) {
    await ensureCleanDir(scriptsTarget);
    await mkdir2(scriptsTarget, { recursive: true });
    await cp2(scriptsSource, scriptsTarget, { recursive: true });
  }
}
async function listBundledSubSkills() {
  const skillsSource = join2(ASSETS_DIR, "skills");
  if (!await exists2(skillsSource))
    return [];
  const entries = await readdir2(skillsSource, { withFileTypes: true });
...
async function copySubSkills(skillsParentDir, force) {
  ...
  for (const name of await listBundledSubSkills()) {
    const target = join2(skillsParentDir, name);
    if (await exists2(target) && !force)
      continue;
    await mkdir2(target, { recursive: true });
    await cp2(join2(skillsSource, name), target, { recursive: true });
  }
}
async function generatePlatformFiles(targetDir, aiType, isGlobal = false, force = false) {
  const config = await loadPlatformConfig(aiType);
  const createdFolders = [];
  const effectiveDir = isGlobal ? homedir() : targetDir;
  const skillDir = join2(effectiveDir, config.folderStructure.root, config.folderStructure.skillPath);
  await mkdir2(skillDir, { recursive: true });
  const skillContent = await renderSkillFile(config, isGlobal);
  const skillFilePath = join2(skillDir, config.folderStructure.filename);
  const fileAlreadyExists = await exists2(skillFilePath);
  ...
  const skillsParentDir = join2(effectiveDir, config.folderStructure.root, dirname(config.folderStructure.skillPath));
  await copySubSkills(skillsParentDir, force);
  return createdFolders;
}
async function generateAllPlatformFiles(targetDir, isGlobal = false, force = false) {
...
function detectAIType(cwd = process.cwd()) {
  const detected = [];
  if (existsSync2(join3(cwd, ".claude"))) {
    detected.push("claude");
  }
  if (existsSync2(join3(cwd, ".cursor"))) {
    detected.push("cursor");
  }
  if (existsSync2(join3(cwd, ".windsurf"))) {
    detected.push("windsurf");
  }
  if (existsSync2(join3(cwd, ".agents")) || existsSync2(join3(cwd, ".agent"))) {
    detected.push("antigravity");
  }
  if (existsSync2(join3(cwd, ".github"))) {
    detected.push("copilot");
  }
  if (existsSync2(join3(cwd, ".kiro"))) {
    detected.push("kiro");
  }
  if (existsSync2(join3(cwd, ".codex"))) {
    detected.push("codex");
  }
  if (existsSync2(join3(cwd, ".roo"))) {
    detected.push("roocode");
  }
  if (existsSync2(join3(cwd, ".qoder"))) {
    detected.push("qoder");
  }
  if (existsSync2(join3(cwd, ".gemini"))) {
    detected.push("gemini");
  }
  if (existsSync2(join3(cwd, ".trae"))) {
    detected.push("trae");
  }
  if (existsSync2(join3(cwd, ".opencode"))) {
    detected.push("opencode");
  }
  if (existsSync2(join3(cwd, ".continue"))) {
    detected.push("continue");
  }
  if (existsSync2(join3(cwd, ".codebuddy"))) {
    detected.push("codebuddy");
  }
  if (existsSync2(join3(cwd, ".factory"))) {
    detected.push("droid");
  }
  if (existsSync2(join3(cwd, ".kilocode"))) {
    detected.push("kilocode");
  }
  if (existsSync2(join3(cwd, ".warp"))) {
    detected.push("warp");
  }
  if (existsSync2(join3(cwd, ".augment"))) {
...
function getAITypeDescription(aiType) {
  switch (aiType) {
    case "claude":
...
// src/utils/logger.ts
var logger = {
  info: (msg) => console.log(source_default.blue("info"), msg),
  success: (msg) => console.log(source_default.green("success"), msg),
  warn: (msg) => console.log(source_default.yellow("warn"), msg),
  error: (msg) => console.log(source_default.red("error"), msg),
  title: (msg) => console.log(source_default.bold.cyan(`
${msg}
`)),
...
var REPO_OWNER = "nextlevelbuilder";
var REPO_NAME = "ui-ux-pro-max-skill";
var API_BASE = "https://api.github.com";
var USER_AGENT = "ui-ux-pro-max-cli";
...
function getGitHubTokenGuidance() {
...
async function downloadRelease(url, dest, token) {
  const response = await fetch(url, {
    headers: {
      "User-Agent": USER_AGENT,
  ...
  await writeFile2(dest, Buffer.from(buffer));
}
...
var ASSETS_CANDIDATES2 = [
  join4(__dirname3, "..", "assets"),
  join4(__dirname3, "..", "..", "assets")
];
var ASSETS_DIR2 = ASSETS_CANDIDATES2.find((path) => existsSync3(path)) ?? ASSETS_CANDIDATES2[0];
async function tryGitHubInstall(targetDir, aiType, spinner, token) {
  ...
  try {
    ...
    const zipPath = join4(tempDir, "release.zip");
...
async function templateInstall(targetDir, aiType, spinner, isGlobal = false, force = false) {
  spinner.text = isGlobal ? "Generating skill files globally..." : "Generating skill files from templates...";
  if (aiType === "all") {
...
async function initCommand(options) {
  ...
  if (!aiType) {
    ...
    if (detected.length > 0) {
      logger.info(`Detected: ${detected.map((t) => source_default.cyan(t)).join(", ")}`);
    }
  ...
  logger.info(`Installing for: ${source_default.cyan(getAITypeDescription(aiType))}${modeLabel}`);
  ...
  try {
    if (options.legacy) {
      ...
      if (installMethod !== "github") {
        ...
        copiedFolders = await copyFolders(ASSETS_DIR2, cwd, aiType);
    ...
    copiedFolders.forEach((folder) => {
      console.log(`  ${source_default.green("+")} ${folder}`);
    });
    console.log();
    logger.success("UI/UX Pro Max installed successfully!");
    console.log();
    console.log(source_default.bold("Next steps:"));
    console.log(source_default.dim("  1. Restart your AI coding assistant"));
    console.log(source_default.dim('  2. Try: "Build a landing page for a SaaS product"'));
    console.log();
  } catch (error) {
...
`);
    console.log(source_default.bold(`Available versions:
`));
    releases.forEach((release, index) => {
      ...
      if (isLatest) {
        console.log(`  ${source_default.green("*")} ${source_default.bold(tag)} ${source_default.dim(`(${date})`)} ${source_default.green("[latest]")}`);
      } else {
        console.log(`    ${tag} ${source_default.dim(`(${date})`)}`);
      }
    });
    console.log();
...
var CLI_PACKAGE_NAME = "ui-ux-pro-max-cli";
async function getPackageVersion() {
  const packagePath = join5(__dirname4, "..", "package.json");
...
async function updateCommand(options) {
  ...
  try {
    ...
    spinner.succeed(`Latest version: ${source_default.cyan(release.tag_name)}`);
    if (currentVersion !== latestVersion) {
      console.log();
      if (!/^\d+\.\d+\.\d+([.-][0-9A-Za-z.-]+)?$/.test(latestVersion)) {
        logger.warn(`Installed CLI is ${source_default.cyan(currentVersion)}; latest release is ${source_default.cyan(release.tag_name)}.`);
        logger.info(`Update the CLI package: ${source_default.cyan(`npm install -g ${CLI_PACKAGE_NAME}@${latestVersion}`)}`);
        logger.info("Then rerun: uipro init --ai <platform> --force");
        return;
      }
      logger.info(`Updating CLI from ${source_default.cyan(currentVersion)} to ${source_default.cyan(latestVersion)}...`);
      console.log();
      const isWindows = process.platform === "win32";
      try {
        ...
        logger.info(`Update manually: ${source_default.cyan(`npm install -g ${CLI_PACKAGE_NAME}@${latestVersion}`)}`);
        process.exit(1);
      }
      console.log();
      logger.success(`Updated to ${source_default.cyan(latestVersion)}.`);
      logger.info(`Now rerun ${source_default.cyan("uipro init --ai <platform> --force")} to refresh your skill files.`);
      return;
    }
    console.log();
...
import { rm as rm3, stat } from "node:fs/promises";
...
async function removeSkillDir(baseDir, aiType) {
  ...
  try {
    ...
    parents.add(join6(folderStructure.root, dirname4(folderStructure.skillPath)));
  } catch {}
  for (const folder of AI_FOLDERS[aiType]) {
    parents.add(join6(folder, "skills"));
  }
  for (const parent of parents) {
    for (const name of skillNames) {
      const skillDir = join6(baseDir, parent, name);
      try {
        await stat(skillDir);
...
async function uninstallCommand(options) {
  ...
  const isGlobal = !!options.global;
  const baseDir = isGlobal ? homedir2() : process.cwd();
  ...
  if (!aiType) {
    ...
    logger.info(`Detected installations: ${detected.map((t) => source_default.cyan(t)).join(", ")}`);
    const choices = [
      ...detected.map((type) => ({
        title: getAITypeDescription(type),
        value: type
  ...
  const { confirmed } = await import_prompts2.default({
    type: "confirm",
    name: "confirmed",
    message: `Remove UI/UX Pro Max skill for ${source_default.cyan(getAITypeDescription(aiType))} from ${locationLabel}?`,
    initial: false
  });
  if (!confirmed) {
    logger.warn("Uninstall cancelled");
  ...
  try {
    ...
    allRemoved.forEach((folder) => {
      console.log(`  ${source_default.red("-")} ${folder}`);
    });
    console.log();
...
var __dirname5 = dirname5(__filename2);
var pkg = JSON.parse(readFileSync(join7(__dirname5, "../package.json"), "utf-8"));
var program2 = new Command;
program2.name("uipro").description("CLI to install UI/UX Pro Max skill for AI coding assistants").version(pkg.version);
program2.command("init").description("Install UI/UX Pro Max skill to current project").option("-a, --ai <type>", `AI assistant type (${AI_TYPES.join(", ")})`).option("-f, --force", "Overwrite existing files").option("-o, --offline", "Compatibility flag; template installs use bundled assets").option("-g, --global", "Install globally to home directory (~/) instead of current project").option("-t, --token <token>", "GitHub Personal Access Token for higher API rate limits").action(async (options) => {
...
program2.command("versions").description("List available versions").option("-t, --token <token>", "GitHub Personal Access Token for higher API rate limits").action(versionsCommand);
program2.command("update").description("Update UI/UX Pro Max to latest version").option("-a, --ai <type>", `AI assistant type (${AI_TYPES.join(", ")})`).option("-t, --token <token>", "GitHub Personal Access Token for higher API rate limits").action(async (options) => {
  if (options.ai && !AI_TYPES.includes(options.ai)) {
    console.error(`Invalid AI type: ${options.ai}`);
    console.error(`Valid types: ${AI_TYPES.join(", ")}`);
    process.exit(1);
...
program2.command("uninstall").description("Remove UI/UX Pro Max skill from current project or globally").option("-a, --ai <type>", `AI assistant type (${AI_TYPES.join(", ")})`).option("-g, --global", "Uninstall from home directory (~/) instead of current project").action(async (options) => {
  if (options.ai && !AI_TYPES.includes(options.ai)) {
    console.error(`Invalid AI type: ${options.ai}`);
    console.error(`Valid types: ${AI_TYPES.join(", ")}`);
    process.exit(1);
