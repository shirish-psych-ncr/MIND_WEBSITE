...
- **NO LIGHTER INPUTS ON DARK BACKGROUNDS:** Inputs are "inset" containers that receive content. They must be slightly _darker_ than their surrounding surface to visually recede and signal "type here".
...
To truly eradicate the "vibe-coded" look, you must understand _why_ the AI generates it. AI does not design; it statistically averages. It regurgitates the median of Dribbble, Behance, and 2021 SaaS templates. By mapping the connections between AI's training biases and human cognitive failures, we can establish the ultimate negative constraints.

---

### 1. THE COGNITIVE & INFORMATION ARCHITECTURE TRAPS

_The Connection: AI equates "whitespace" with "premium." It optimizes for the visual aesthetic of a marketing site, completely ignoring the cognitive load required for actual product usage._

- **DO NOT default to "Clean and Modern" for data-heavy tools.**
  - _The Trap:_ AI interprets "clean" as "empty." It will give you massive padding and low information density.
  - _The Reality:_ A trading floor, an SRE dashboard, or a video editor requires _high density_. Whitespace is a luxury for consumer apps; density is a feature for productivity tools.
  - _The Fix:_ Force the AI to define density based on the user persona. (e.g., "Dense like a terminal," vs. "Calm like a reading app").
- **DO NOT treat Navigation as "Scaffolding."**
  - _The Trap:_ AI builds a generic left-sidebar because "SaaS apps have sidebars." It treats navigation as a structural requirement, not a psychological map.
  - _The Reality:_ Navigation _is_ the product. It teaches the user how to think about the space. A page floating in space without contextual grounding feels like a component demo, not software.
  - _The Fix:_ Navigation must emerge from the user's core verb (e.g., "Grade submissions," "Approve payments"), not a generic "Dashboard/Settings/Profile" template.
- **DO NOT design for the "Happy Path" (The Linguistic Edge-Case Failure).**
  - _The Trap:_ AI designs cards and headers for perfect, 3-word English headlines.
  - _The Reality:_ Real UI breaks instantly. It fails on 60-character names, German compound words, 500,000+ numerical values, and empty states.
  - _The Fix:_ Mandate "Stress-Test Prompting." Force the AI to render the UI using worst-case data (e.g., "Use a 50-character string for the title, a 9-digit number for the metric, and a 3-line error message").
- **DO NOT use "Stream of Consciousness" Information Architecture.**
  - _The Trap:_ AI dumps every feature, metric, and CTA onto a single viewport because it lacks the concept of "pacing."
  - _The Reality:_ It violates Hick’s Law and overwhelms the user.
  - _The Fix:_ Enforce "Drip-Feed Architecture." The UI must progressively disclose complexity. The primary action must be isolated from secondary noise.

---

### 2. THE SEMANTIC & CONTEXTUAL VOIDS

_The Connection: AI lacks "Theory of Mind." It doesn't know who is looking at the screen, what they did 5 minutes ago, or what they will do 5 minutes from now. It generates interfaces in a vacuum._

- **DO NOT use Contradictory or "AI-Hedging" Copywriting.**
  - _The Trap:_ AI generates phrases like "Up to 5+ turns," "Seamless integration," or "Welcome to our platform."
  - _The Reality:_ It sounds robotic, legally defensive, and meaningless.
  - _The Fix:_ Ban all marketing hedging. Copy must be specific, actionable, and human. (e.g., Change "Optimize your workflow" to "Cut rendering time by 40%").
- **DO NOT ignore the "Temporal Context" of the User.**
  - _The Trap:_ AI designs for a user sitting in a coffee shop on a Sunday.
  - _The Reality:_ What if the user is an SRE waking up at 3 AM to a server crash? What if they are a founder rushing between investor meetings?
  - _The Fix:_ Define the _environmental context_ in your design system. High-stress contexts require high-contrast, large touch targets, and zero decorative animations.
- **DO NOT use Positional Tokens for Semantic Meaning.**
  - _The Trap:_ AI uses `bg-gray-100` for an input field and `bg-white` for a card.
  - _The Reality:_ When you switch to Dark Mode, or change the brand theme, the entire UI shatters because the colors are tied to _lightness_, not _intent_.
  - _The Fix:_ Ban raw color/positional tokens in component logic. Use semantic intent tokens: `--surface-inset`, `--surface-elevated`, `--text-primary`, `--border-separator`.

---

### 3. THE STRUCTURAL & SYSTEMIC CODE ROT

_The Connection: AI writes CSS to "make it work right now," not to build a scalable design system. This results in "Frankenstein UIs" that look fine on page 1, but fracture by page 10._

- **DO NOT mix Depth Strategies within the same viewport.**
  - _The Trap:_ AI will use a drop-shadow on a card, a border on a button, and a background-color shift on a sidebar.
  - _The Reality:_ It creates visual noise. The user's eye doesn't know which tool is being used to establish hierarchy.
  - _The Fix:_ Pick ONE elevation language per project and enforce it globally. (e.g., "We use _only_ 1px hairline borders for separation. Zero drop shadows anywhere.")
- **DO NOT animate for "Decoration" (The Micro-Interaction Abuse).**
  - _The Trap:_ AI thinks animation = polish. It adds 500ms fade-ins, springy modals, and skeleton loaders for instant local state changes.
  - _The Reality:_ It feels sluggish, uncanny, and disrespectful of the user's time. It screams "I am an AI trying to look fancy."
  - _The Fix:_ Motion must be _functional_ (orienting the user, confirming an action). It must use deceleration easing and settle in under 200ms. If it takes longer than 200ms, it is unnecessary.
- **DO NOT rely on the "Dribbble UI" Aesthetic (Form Over Function).**
  - _The Trap:_ AI mimics Dribbble shots: overzealous padding, massive typography, terrible practical contrast, and visually pretty but functionally unusable layouts.
  - _The Reality:_ It looks great in an Instagram post but gets you fired in production.
  - _The Fix:_ Apply the "Squint Test." If you blur your eyes and cannot instantly perceive the hierarchy and primary action, the UI has failed.

---

### 4. THE "ANTI-SLOP" DIAGNOSTIC FRAMEWORK (MANDATORY CHECKS)

_Inject these specific diagnostic tests into your AI agent's reasoning process. Force the AI to evaluate its own output against these rules before generating code._

- **THE SWAP TEST:**
  - _Prompt:_ "If you swapped your chosen typography, layout, and color palette for the most common AI defaults (Inter font, purple/blue gradient, 3-column card grid), would the design feel meaningfully different? If no, you have defaulted. Refactor for specific intent."
- **THE SIGNATURE TEST:**
  - _Prompt:_ "Can you point to three specific, structural elements in this UI that could _only_ exist for this specific product? If the UI could belong to any generic SaaS, you have failed. Inject domain-specific metaphors and structures."
- **THE TOKEN TEST:**
  - _Prompt:_ "Read your CSS variables out loud. Do they sound like they belong to this product's specific world (e.g., `--ink`, `--parchment`, `--terminal-green`), or could they belong to any project (`--gray-700`, `--surface-2`)? Rename them to reflect the product's domain."
- **THE BOUGIE DESIGN AESTHETICS (BDA) CHECK:**
  - _Prompt:_ "Are you using disproportionate vector illustrations (tiny heads, giant limbs), generic 'Corporate Memphis' characters, or abstract 3D shapes floating in space? Ban all of these. Use authentic data visualizations, strict geometric grids, or real photography."

---

### 🧠 SYSTEM PROMPT INJECTION: THE "ANTI-EPISTEMOLOGY" DIRECTIVE

_(Copy/Paste this directly into your AI Agent's core instructions to enforce deep structural discipline)_
...
If the AI catches itself defaulting to _any_ of these patterns, it must immediately stop, recognize the statistical mean it is falling into, and refactor for deliberate, domain-specific intent.

---

#### 1. THE FORBIDDEN AESTHETIC (The "AI Slop" Tells)

_These are the visual signatures of an AI guessing the "mean" of the internet. They are strictly prohibited._
...
- **NO Mixed Depth Strategies:** Never mix drop shadows, 1px borders, and surface-color shifts in the same view. Pick ONE elevation language (e.g., _only_ hairline borders, or _only_ subtle surface shifts) and commit globally.
...
_AI defaults to a "kitchen sink" approach with arbitrary spacing. These structural habits are forbidden._
...
_AI lacks a sense of voice and defaults to corporate hedging or flat hierarchy. These are forbidden._
...
- **NO AI Prose Tics:** Ban the use of em-dashes (`—`), triadic auto-pilot lists, and hollow confidence words like _load-bearing, robust, delve, empower, tapestry, data-driven, pivotal, in today's, let's dive in, morever, furthermore_.

#### 4. THE COMPONENT & MOTION RED LINES (The "Unfinished" Tells)

_AI builds for the "Happy Path" and ignores physics. These interaction failures are forbidden._
...
- **NO Lighter Inputs on Dark Backgrounds:** Inputs are "inset" containers. They must be slightly _darker_ than their surrounding surface to visually recede and signal "type here".
- **NO Bounce, Spring, or Elastic Easing:** Professional interfaces do not bounce. Use fast micro-interactions with `ease-out` (deceleration) for entrances, and `ease-in` for exits.
...
_AI designs in a vacuum, ignoring the user's environment and cognitive load._
...
_(Copy/Paste this directly into your AI Agent's core instructions. Force the AI to run these diagnostic checks before outputting final code.)_
...
- _Cognitive Load:_ Novice, exploratory, low time-pressure
- _Grid:_ `16px` base unit
- _Vertical Rhythm:_ `48px–64px`
- _Typography:_ Body `16px/1.6`, Headings `1.333` scale
- _Color:_ Low chroma backgrounds (`OKLCH < 15`), high whitespace ratio
- _Motion:_ `200ms–300ms` fade/slide, `ease-out`
- _Layout:_ Single primary CTA per viewport, generous card gutters

**TIER 2: MEDIUM DENSITY (SaaS Dashboard / Productivity)**

- _Cognitive Load:_ Competent, routine, moderate time-pressure
- _Grid:_ `8px` base unit
- _Vertical Rhythm:_ `24px–32px`
- _Typography:_ Body `14px–15px/1.5`, Headings `1.25` scale, strict weight pairing
- _Color:_ Semantic tints only. Accent chroma capped at `40`. Backgrounds shift by `3–5%` lightness
- _Motion:_ `120ms–180ms`, transform/opacity only
- _Layout:_ 2–3 data panels per viewport, inline actions, sticky headers

**TIER 3: HIGH DENSITY (Enterprise / Terminal / Trading / IDE)**

- _Cognitive Load:_ Expert, high stakes, time-critical, high focus
- _Grid:_ `4px` base unit
- _Vertical Rhythm:_ `12px–16px`
- _Typography:_ Monospace or compact sans. Body `13px/1.4`, tight tracking (`-0.01em`)
- _Color:_ Near-zero decorative color. High contrast (`7:1+` explicit). Borders over shadows
- _Motion:_ `0ms` or `80ms` max. No easing. Instant state changes
- _Layout:_ Tabular data, inline editing, keyboard shortcuts, zero whitespace padding on non-critical elements
...
_(Copy/paste directly into `.cursorrules`, `CLAUDE.md`, or system prompt. This forces the AI to operate on measurable parameters, not vibes.)_
